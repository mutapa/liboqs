...
aeskey_t aeskey_init_RLCE(unsigned short kappa) {
...
void aeskey_free_RLCE(aeskey_t key) {
  free(key->key);
  free(key);
  return;
}
...
void AES_encrypt_RLCE(unsigned char plain[], unsigned char cipher[], aeskey_t key) {
  int k;
  unsigned char *w;
...
void AES_decrypt_RLCE(unsigned char cipher[], unsigned char plain[], aeskey_t key) {
  int i;
  unsigned char *w;
  w=calloc(key->wLen, sizeof(unsigned char));
  if ((key->Nk) ==4) {
